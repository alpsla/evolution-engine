...
class TestUniversalPatternSync:
    """Tests for auto-import of bundled universal patterns."""

    def test_import_universal_patterns(self, tmp_path):
        """The bundled universal_patterns.json should be importable."""
        ...
        # Load the actual bundled file
        data_dir = Path(__file__).parent.parent.parent / "evolution" / "data"
        universal_path = data_dir / "universal_patterns.json"
        if not universal_path.exists():
            pytest.skip("No universal_patterns.json bundled")
    ...
    def test_import_universal_idempotent(self, tmp_path):
        """Importing the same universal patterns twice should skip all on second run."""
        from evolution.kb_export import import_patterns
    ...
    def test_orchestrator_auto_imports(self, tmp_path, monkeypatch):
        """Orchestrator._import_universal_patterns should import bundled patterns."""
        from evolution.orchestrator import Orchestrator

        # Create a minimal evo_dir with a knowledge.db
        ...
        # Create orchestrator with a dummy repo path
        orch = Orchestrator.__new__(Orchestrator)
        orch.evo_dir = evo_dir

        messages = []
        imported = orch._import_universal_patterns(lambda msg: messages.append(msg))

        # Should import at least 1 pattern (from universal_patterns.json)
        data_dir = Path(__file__).parent.parent.parent / "evolution" / "data"
        if (data_dir / "universal_patterns.json").exists():
            data = json.loads((data_dir / "universal_patterns.json").read_text())
            expected = len(data.get("import_ready", []))
            if expected > 0:
                assert imported == expected

                # Verify patterns are in the KB
                kb = SQLiteKnowledgeStore(db_path)
                community = kb.list_patterns(scope="community")
                assert len(community) == expected
                kb.close()
